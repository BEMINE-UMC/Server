...
| 회원가입 및 로그인, 인증 기능(이메일 인증, 이메일 찾기, 비밀번호 재설정, 중복 검사), 게시물 조회(좋아요, 다른 게시물) | 템플릿 좋아요 순 조회 , 연혁수정 , 게시글 작성/수정 , 게시글 삭제 , 게시글 이미지 삽입 , 게시글 상세조회 ,   | 템플릿 좋아요, 게시물 좋아요, 게시물 북마크, 게시물 검색, 사용자 작성 포스트 조회, 이메일 찾기 |  연혁 조회 , 최근 본 게시물 조회 , 북마크한 게시물 조회, 프로필 수정, 템플릿 수정, 템플릿 생성, 연혁 생성, 사용자 정보 조회, 자기소개 수정 | 게시물 목록 조회, 템플릿 목록 조회, 템플릿 단일 조회(업로드 화면, 파일 조회 창), 템플릿 삭제 |
